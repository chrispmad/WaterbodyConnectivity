...
lake_networks = read_csv("C:/Users/CMadsen/Downloads/LocalRWork/output/provincial_waterbody_networks.csv")
...
eg_streams = bcdc_query_geodata('freshwater-atlas-stream-network') %>% 
  filter(INTERSECTS(st_bbox(eg_lakes %>% filter(GNIS_NAME_1 == "Mara Lake")))) %>% 
  collect()
...
eg_unioned = st_cast(st_union(eg_lakes %>% 
                   ...
                   bind_rows(
                     st_buffer(
                       ...
                       1) %>% 
                       select(GNIS_NAME) %>% 
                       rename(GNIS_NAME_1 = GNIS_NAME)
                     )
                 ), "POLYGON")
eg_unioned_df = data.frame(network = c(1,2))
  
eg_unioned = st_set_geometry(eg_unioned_df, eg_unioned)

eg_plot_left = ggplot() + 
  geom_sf(data = eg_streams, col = "grey") +
  geom_sf(data = eg_lakes %>% st_crop(st_buffer(eg_lakes %>% filter(GNIS_NAME_1 == "Mara Lake"), 10000)), aes(fill = GNIS_NAME_1)) + 
  geom_sf_label(data = eg_lakes %>% st_crop(st_buffer(eg_lakes %>% filter(GNIS_NAME_1 == "Mara Lake"), 10000)), aes(label = GNIS_NAME_1)) +
  #theme_map() + 
  labs(fill = "Lake Name",
...
```{r biggest component, fig.width = 12, fig.height=10}
# big_comp_eg = bcdc_query_geodata('freshwater-atlas-lakes') %>% 
#   filter(WATERBODY_KEY %in% all_of(lake_networks %>% filter(network_number == 14547) %>% pull(WATERBODY_KEY))) %>% 
#   # filter(GNIS_NAME_1 %in% all_of(lake_networks %>% filter(network_number == 14547) %>% pull(GNIS_NAME_1))) %>% 
#   collect()
# 
# big_comp_eg %>% filter(GNIS_NAME_1 %in% all_of(lake_networks %>% filter(network_number == 14547) %>% pull(GNIS_NAME_1)))
# big_comp_subws = lake_networks %>% 
#                                    filter(network_number == 14547) %>% 
#                                    distinct(WATERSHED_GROUP_ID) %>% 
#                                    pull(WATERSHED_GROUP_ID)
# 
# ggplot() + 
#   geom_sf(data = subw %>% filter(WATERSHED_ %in% big_comp_subws)) +
#   geom_sf(data = big_comp_eg)
```

#### 1. Constellation of Subwatershed Components
...
Of particular note is the component centered around subwatershed 167. Also, can you spot the hexagram??
...
# precross_df = graph_df %>% 
#   #Add leading zeros to the subwatershed group id.
#   mutate(WATERSHED_GROUP_ID = str_pad(WATERSHED_GROUP_ID,3,pad = "0")) %>% 
#   #Flag any networks that are present in the subwatershed of interest.
#   group_by(network_number) %>% 
#   mutate(subw_by_network = paste0(WATERSHED_GROUP_ID, collapse = ", ")) %>%
#   #Only retain networks that join 2+ subwatersheds
#   filter(str_detect(subw_by_network, ", ")) 
#   
# #Cycling through the subwatershed ID numbers, we find any networks that include 
# # the subwatershed in question to generate a 2-column matrix that links the linkages of subwatersheds.
# for(i in 1:246){
#   subw_number = str_pad(i, width = 3, pad = "0")
#   
#   crossing_per_subw = precross_df %>% 
#     mutate(this_subw = str_detect(subw_by_network, subw_number)) %>% 
#     filter(this_subw == T) %>% 
#     ungroup() %>% 
#     filter(!duplicated(WATERSHED_GROUP_ID)) %>% 
#     expand(from = WATERSHED_GROUP_ID,
#          to = WATERSHED_GROUP_ID) %>% 
#     filter(from != to) %>% 
#     select(from, to) %>% 
...
```{r circle_graph_no_singles, fig.width=12, fig.height=10} 
# If we exclude subwatersheds that are not connected to other subwatersheds by their waterbody networks, we get the following graph.
