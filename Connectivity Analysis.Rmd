...
#First, just to be generous, buffer the edge networks by 5 more meters (total buffer now 6m)
all_edge_networks_buffered = st_buffer(all_edge_networks, dist = 5)
...
overlaps_to_add_to_output_table = overlaps_to_add_to_output_table %>% 
  ...
  separate(network_number_right, sep = ", ", into = c(paste0("network_number_right_",rep(1:8))))
```
...
lake_network_replace_table = lake_networks %>% 
  ...
  dplyr::select(-network_number) %>% 
  left_join(network_number_old_to_new %>% rename(network_number_right_8 = network_number_old)) %>% 
  mutate(network_number_right_8 = network_number) %>% 
  dplyr::select(-network_number) %>% 
