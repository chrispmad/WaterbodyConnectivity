...
# ---------------------------------------------------------- #
# This script analyses all of the streams, rivers and lakes in British Columbia to identify networks, or "components" , of spatially linked lakes. The motivation for this analysis is principally to inform risk analyses for aquatic invasive species; for example, if a goldfish were released in lake A, what are all of the lakes to which this invasive fish species could potentially travel? Note that this analyses does not currently account for elevational differences between lakes or flow rates.
# ---------------------------------------------------------- #
...
if(starting_number > 1 & file.exists("lakes_network_table.csv")){
...
for(i in starting_number:nrow(regions)){
  ...
  #Query and temporarily download streams and lakes for the i'th subwatershed.
  ...
  #Trim away and streams and lakes that aren't actually inside the i'th subwatershed.
  ...
  #Buffer streams - we do this to enable an overlap analysis between streams (and lakes!). It changes the geometry type of the streams from linestring to polygon.
  tic()
  streams = st_buffer(streams, dist = 1)
  #print("Buffering of connector shapefile complete.")
  toc()

  #Buffer lakes by a little bit (3 meters) to ensure spatial overlap. Otherwise, very small gaps between lakes and streams that connect in reality may result in the analysis not recognizing these overlaps.
  tic()
  ...
  #Strip away all columns except the geometry column for the streams; remove Z coordinates from any streams that have them (these indicate height).
  streams = streams %>% 
    select(geometry) %>% 
    st_zm()
    
  #Find the number of streams or rivers that connect to lakes.
  number_connections_table = as.data.frame(st_intersects(lakes, streams)) %>% 
    as_tibble() %>% 
    group_by(row.id) %>% 
    summarise(number_connections = n())
  
  #Add a new variable to the lakes object - this indicates the number of streams or rivers that touch each lake. We initialize the column with 0s for all lakes.
  ...
  tic()
  #This is the core element of this analysis loop - for any lakes and streams / rivers that overlap, we join those polygons together into networks.
...
#First, just to be generous, buffer the edge networks by 2 more meters (total buffer now 3m). This may join together streams whose geometries don't otherwise come in contact with their 'partners' across the subwatershed boundaries.
